...
export const SolicitudesClient = ({
  solicitudes,
  pendientes,
  TABS,
  estadoValido,
        ...
        <div className="inline-flex gap-1 mb-4 bg-white p-1 rounded border border-[#e5e7eb] shadow-sm">
          {TABS.map((tab: any) => {
            const isActive =
              tab.value === "TODAS"
                ? !estadoValido
                : estadoValido === tab.value;
            const href =
              tab.value === "TODAS"
                ? "/dashboard/solicitudes"
                : `/dashboard/solicitudes?estado=${tab.value}`;
            return (
              <Link
                key={tab.value}
                href={href}
              ...
              <table className="w-full text-sm">
                <thead className="bg-[#f3f3f6]">
                  <tr className="border-b border-[#e5e7eb]">
                    ...
                    <th className="text-left py-3 px-4 text-[10px] font-black uppercase tracking-widest text-[hsl(var(--muted-foreground))] hidden lg:table-cell">
                      Vehículo
                    </th>
                    <th className="text-left py-3 px-4 text-[10px] font-black uppercase tracking-widest text-[hsl(var(--muted-foreground))]">
                      Financiación
                  ...
                  {solicitudes.map((c: any) => (
                    ...
                    >
                      <td className="py-3 px-4">
                        ...
                        <Link
                          href={`mailto:${c.email}`}
                          className="text-xs font-medium text-[hsl(var(--primary))] hover:underline block mt-0.5"
                        >
                          {c.email}
                        </Link>
                        {c.telefono && (
                          <Link
                            href={`tel:${c.telefono}`}
                            className="text-xs font-medium text-[hsl(var(--muted-foreground))] hover:underline block mt-0.5"
                          >
                            {c.telefono}
                          </Link>
                        )}
